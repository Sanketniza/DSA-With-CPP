...
vector<vector<int>> nQueens(int n) {
    ...
    // Initializing the board with 0s
    //  0 means empty cell, 1 means queen is placed
    vector<vector<int>> ans;

    solve(0, board, ans, n);
    // solve(0 -> col, board , ans, n);
    // Base case is when col == n, then we add the solution to ans
